        ...
        switch(action.type){
            case "INIT":
                return {...state, //...state - старое состояние
                    ...
                    commentsNew:action.commentsNew,
                    user: action.user,
            ...
            case "POST_ADD": // Беру из action, внутренности показывают как будет выглядеть пост
                ...
                return {...state, //...state - старое состояние
                        ...
                        // comments: [...state.posts, comments],  // В store объект posts в виде старого состояния, newPost - новое состояние (обозначено выше в reducer)
                        lastIdPost: newId //Возврат id
                        }; 
            case "COMMENT_ADD": // Беру из action, внутренности показывают как будет выглядеть пост
                let newIdComment = ++state.lastIdPost; // Если убрать ++ - при нажатии на удаление, удаляет все посты 
                let comments  = {
                    id: newIdComment, // Добавил убралась ошибка с key
                    author: action.post.name, // Передаем из Экшена свойство name
                    text: action.post.text, // Передаем из Экшена свойство text
                    };

                return {...state, //...state - старое состояние
                        posts: [...state.posts, comments],
                        lastIdPost: newId //Возврат id
                        }; 
